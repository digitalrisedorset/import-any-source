...
import {WocommerceApiProducteResponse} from '../types/woocommerce'

export function RemoteWoocommerceProductProvider() {
    ...
    const loadProducts = async function() {
        try {
            ...
            const response: WocommerceApiProducteResponse = await Axios.get('/getWoocommerceProductList', { cancelToken: ourRequest.token });
